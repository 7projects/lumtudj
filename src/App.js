...
import { AddAlertRounded, Album, AlignVerticalCenterTwoTone, Favorite, PlaylistAdd, Timelapse } from '@mui/icons-material';
...
function App() {
  ...
  const [plcFilterTerms, setPlcFilterTerms] = useState([]);

  const [mainActivityIndex, setMainActivityIndex] = useState(0);
  const [mainActivities, setMainActivities] = useState([{ name: "", tracks: [] }]);

  useEffect(() => {
    if (plcFilter && plcFilter.trim() != "") {
      const terms = plcFilter.toLowerCase().split(",").filter(t => t.trim() != "");
      setPlcFilterTerms(terms);
    } else {
      setPlcFilterTerms([]);
    }
  }, [plcFilter]);
  ...
  const onPanelMainToolbarButtonClick = (action) => {
    switch (action) {
      ...
      case "LastListened":
        ...
        break;
      case "TopTracks":
        loadPlaylistPrev({ id: "TopTracks", name: "Top 10 Tracks", type: "toptracks" });
        break;
      case "search":
  ...
  const onPlaylistContextMenu = (e, track, index) => {
    ...
    items.push({ label: "Artist info", onClick: () => { setSelectedTrack(track); loadArtistInfo(track); setShowArtistInfo(true) }, icon: <PersonIcon /> });
    items.push({ label: "Top 10 tracks", onClick: () => { setSelectedTrack(track); loadArtistTopTracks(track); }, icon: <StarsIcon /> });
    items.push({ label: "Albums", onClick: () => { setSelectedTrack(track); loadArtistAlbums(track); }, icon: <Album /> });
    items.push({ label: "Track album", onClick: () => { setSelectedTrack(track); loadPlaylistPrev(track.album); }, icon: <Album /> });
                      ...
                      {tab != 4 ? <td colSpan={4} className='tab-panel'>
                        ...
                        <Activity mode={tab == "plprev" ? "visible" : "hidden"}>
                          <div className="toolbar-wrapper">
                            <input ref={inputRef} className="toolbar-input-search" placeholder="search library and albums..." onFocus={(e) => e.target.select()} value={selectedLibraryItem && selectedLibraryItem.name} onChange={(e) => onPlaylistFilterChange(e.target.value)} />
                          </div>
                          ...
                          <td className='selected-track-container'>
                            {library
                              ...
                              .map(p =>
                                selectedTrack &&
                                  p.tracks &&
                                  p.tracks.some(t => t.id === selectedTrack.id) ? (
                                  <span
                                    key={p.id}
                                    onContextMenu={(e) => onLibraryItemContextMenu(e, p)}
                                    onClick={() => addToSpotifyPlaylist(p, true)}
                                    className='selected-track-bulb-on'
                                  >
                                    {p.name}
                                  </span>
                                ) : (
                                  <span
                                    key={p.id}
                                    onContextMenu={(e) => onLibraryItemContextMenu(e, p)}
                                    onClick={() => addToSpotifyPlaylist(p, false)}
                                    className='selected-track-bulb-off'
                                  >
                                    {p.name}
                                  </span>
                                )
