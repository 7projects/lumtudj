...
const PanelMain = ({ enableDrag, onAlbumClick, activityIndex, allowDrop, onNewActivity, onBack, onBulbsClick, onForward, onToolBarClick, onChange, selectedLibraryItem, tracks, onContextMenu, mode, handleMenu, onDoubleClick, isLocked, onClick, onMenuClick, onSwipedRight, onBulbClick, onBulbCheckClick, onLongPress, onShuffleClick, onDrop }) => {

    const { setMenuAnchor, loadingTracks, setLoadingTracks, library, setLoadingLibrary, filteredLibrary, setFilteredLibrary, setLibrary, loadingLibrary, selectedLibraryIndex, setSelectedLibraryIndex, backgroundPlaylists, setBackgroundPlaylists, selectedTrack, setSelectedTrack, selectedTrackIndex, setSelectedTrackIndex } = useAppStore();
    ...
    useEffect(() => {
        if (!libraryItem?.images && inputRef.current) {
    ...
    const onSearchClick = () => {
        ...
        onNewActivity && onNewActivity(playlist);
    }

    useEffect(() => {

        // alert("selectedLibraryItem changed");
    ...
    const search = async (query) => {
        ...
        onNewActivity && onNewActivity(na);
    };
    ...
    const handleSelectedPlaylistDragEnd = (event) => {
        ...
        if (active.id !== over.id) {
            const oldIndex = selectedPlaylistTracks.findIndex((i, index) => "spl" + activityIndex + index + "-" + i.id === active.id);
            const newIndex = selectedPlaylistTracks.findIndex((i, index) => "spl" + activityIndex + index + "-" + i.id === over.id);
                ...
                {true ?
                    <>
                        <div className='toolbar-search'>
                            ...
                            <ArrowForwardIosIcon className={onForward ? 'toolbar-button' : 'toolbar-button-disabled'} onClick={onForward}></ArrowForwardIosIcon>

                            {/* <KeyboardArrowRightIcon></KeyboardArrowRightIcon> */}
                            <SearchIcon className='toolbar-button' style={{ cursor: "pointer" }} onClick={onSearchClick}></SearchIcon>
                            {libraryItem?.images ? <img className="" style={{ width: 20, borderRadius: "50%" }} src={libraryItem?.images?.[2]?.url || libraryItem?.images?.[0]?.url} />

                                : null}

                            {libraryItem?.images ? <div style={{fontSize: 14}}>{searchText}</div> :
                        ...
                        <div className='toolbar-icons'>
                            ...
                            {/* <Tooltip style={{}} enterDelay={500} title={"Show last shazamed tracks"} > */}
                                <div>
                                    <svg onClick={() => onToolBarClick("MyShazamedTracks")} className='toolbar-button' width="22" height="22" viewBox="0 0 35 35" version="1.1" xmlns="http://www.w3.org/2000/svg">
