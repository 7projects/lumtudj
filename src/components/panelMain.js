...
const PanelMain = ({onBack, selectedLibraryItem, tracks, onContextMenu, mode, handleMenu, onDoubleClick, isLocked, onClick, onMenuClick, onSwipedRight, onBulbClick, onBulbCheckClick, onLongPress, onShuffleClick, onDrop }) => {
                ...
                {mode == "normal" ?
                    <>
                        <div className='toolbar-search'>
                            <KeyboardArrowLeftIcon onClick={onBack}></KeyboardArrowLeftIcon>
                            <KeyboardArrowRightIcon></KeyboardArrowRightIcon>
                            <SearchIcon className='toolbar-button' style={{ cursor: "pointer" }} onClick={() => { setSearchText(""); inputRef.current.focus(); }}></SearchIcon>
                            {libraryItem ? <img className="" style={{ width: 20 }} src={libraryItem?.images?.[2]?.url || libraryItem?.images?.[0]?.url} />
                        ...
                        <div className='toolbar-icons'>
                            ...
                            <Tooltip style={{ zIndex: 9999 }} enterDelay={500} title={"Aggregate tracks across playlists, count how many playlists each song appears in"} >
                                <StarsIcon className='toolbar-button' onClick={() => setSelectedPlaylistTracks(getTracksSortedByPlaylistCount())}></StarsIcon>
                            </Tooltip>
                            <Tooltip style={{ zIndex: 9999 }} enterDelay={500} title={"Show last listened tracks"} >
                                <HistoryIcon className='toolbar-button' onClick={() => loadPlaylistPrev(lastListenedPl)}></HistoryIcon>
                            </Tooltip>
                            <Tooltip style={{ zIndex: 9999 }} enterDelay={500} title={"Show last shazamed tracks"} >
