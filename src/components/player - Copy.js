...
const Player = ({ locked, token, trackid, onClick, playlists, stateChanged, onError, onNext }) => {
    ...
    const timer = async () => {
        ...
        console.log("timer: playerRef.current: " && playerRef.current);
        console.log("timer: playerState: " + playerState);
    ...
    useEffect(() => {
        ...
        //     playerRef.current.setAccessToken(token);

    }, [token]);


    const initializePlayer = () => {
        alert("initialize player");
